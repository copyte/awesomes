...
(function (NodeType) {
    ...
    NodeType[NodeType["Param"] = 2] = "Param";
    NodeType[NodeType["CatchAll"] = 3] = "CatchAll";
                    ...
                    if (node.wildChild) {
                        ...
                        if (path.length >= node.path.length &&
                            ...
                            const prefix = fullPath.slice(0, fullPath.indexOf(pathSeg)) +
                                node.path;
                            throw new Error(`'${pathSeg}' in new path '${fullPath}' conflicts with existing wildcard '${node.path}' in existing prefix '${prefix}'`);
                        }
                    }
                    const c = path[0];
                    // slash after param
                    if (node.nType === NodeType.Param &&
                        c === "/" &&
        ...
        for (let i = 0, max = path.length; numParams > 0; ++i) {
            ...
            while (end < max && path[end] !== "/") {
                switch (path[end]) {
                    ...
                    case ":":
                    case "*":
                        throw new Error(`only one wildcard per path segment is allowed, has: '${path
                            .slice(i)}' in path '${fullPath}'`);
                    default:
                        ++end;
                }
            }
        ...
        walk: while (true) {
            if (path.length > node.path.length) {
                ...
                for (let i = 0; i < node.indices.length; ++i) {
                    if (node.indices[i] === "/") {
                        node = node.children[i];
                        if ((node.handle && node.path.length === 1) ||
